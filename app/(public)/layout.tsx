...
export default async function PublicLayout({
  ...
  return (
    <div className="relative min-h-screen flex flex-col text-wit-white">
          ...
          <nav className="flex gap-1 text-sm">
            {[
              ...
              <Link
                key={item.href}
                href={item.href}
                className="rounded-md px-3 py-1.5 text-wit-muted hover:text-wit-red hover:bg-white/5 transition-colors"
              >
                {item.label}
              </Link>
            ))}
      ...
      <footer className="border-t border-white/10 text-xs text-wit-muted">
